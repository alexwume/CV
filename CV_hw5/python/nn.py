import numpy as np
from util import *
# do not include any more libraries here!
# do not put any code outside of functions!

############################## Q 2.1 ##############################
# initialize b to 0 vector
# b should be a 1D array, not a 2D array with a singleton dimension
# we will do XW + b. 
# X be [Examples, Dimensions]
def initialize_weights(in_size,out_size,params,name=''):
    # W, b = None, None
    high = np.sqrt(6 / (in_size + out_size))
    low = - high
    W = np.random.uniform(low, high, (in_size, out_size))
    b = np.zeros((out_size))
    ##########################
    ##### your code here #####
    ##########################
    params['W' + name] = W
    params['b' + name] = b

############################## Q 2.2.1 ##############################
# x is a matrix
# a sigmoid activation function
def sigmoid(x):
    # res = None

    ##########################
    ##### your code here #####
    ##########################
    res = 1 / (1 + np.exp(-x))

    return res

############################## Q 2.2.1 ##############################
def forward(X,params,name='',activation=sigmoid):
    """
    Do a forward pass

    Keyword arguments:
    X -- input vector [Examples x D]
    params -- a dictionary containing parameters
    name -- name of the layer
    activation -- the activation function (default is sigmoid)
    """
    # pre_act, post_act = None, None
    # get the layer parameters
    W = params['W' + name]
    b = params['b' + name]

    # x = X.T
    # b = b.reshape((b.shape[0],1)
    ##########################
    ##### your code here #####
    ##########################
    # print('W shape', W.shape)
    # print('X shape', X.shape)
    # print('b shape', b.shape)
    # print('np dot X W', np.dot(X,W).shape)
    # print('Xshape', X.shape)
    pre_act = X @ W + b
    post_act = activation(pre_act)

    # store the pre-activation and post-activation values
    # these will be important in backprop
    params['cache_' + name] = (X, pre_act, post_act)

    return post_act

############################## Q 2.2.2  ##############################
# x is [examples,classes]
# softmax should be done for each row
def softmax(x):
    # res = None
    # c = np.zeros((x.shape[0], 1))
    res = np.zeros((x.shape))
    sum = np.zeros((x.shape[0],1))

    c = - np.max(x, axis = 1) ## check dimension

    x = x + np.expand_dims(c, axis = 1)
    x_exp = np.exp(x)
    sum[:,0] = np.sum(x_exp, axis = 1)

    for i in range(x.shape[0]):
        res[i,:] = x_exp[i,:] / sum[i,0]

    ##########################
    ##### your code here #####
    ##########################

    return res

############################## Q 2.2.3 ##############################
# compute total loss and accuracy
# y is size [examples,classes]
# probs is size [examples,classes]
def compute_loss_and_acc(y, probs):
    # loss, acc = None, None

    loss = - np.sum(y * np.log(probs))
    pred_y =  np.argmax(probs, axis=1).reshape((-1,1)) # find the most possible class
    y_label = np.argmax(y, axis = 1).reshape((-1,1))

    acc = np.sum(y_label == pred_y)/y.shape[0]


    ##########################
    ##### your code here #####
    ##########################

    return loss, acc 

############################## Q 2.3 ##############################
# we give this to you
# because you proved it
# it's a function of post_act
def sigmoid_deriv(post_act):
    res = post_act*(1.0-post_act)
    return res

def backwards(delta,params,name='',activation_deriv=sigmoid_deriv):
    """
    Do a backwards pass

    Keyword arguments:
    delta -- errors to backprop
    params -- a dictionary containing parameters
    name -- name of the layer
    activation_deriv -- the derivative of the activation_func
    """
    # grad_X, grad_W, grad_b = None, None, None
    # everything you may need for this layer


    ##check
    W = params['W' + name]
    b = params['b' + name]
    X, pre_act, post_act = params['cache_' + name]

    # do the derivative through activation first
    # then compute the derivative W,b, and X
    ##########################
    ##### your code here #####
    ##########################
    grad_X = np.zeros(X.shape)
    grad_W = np.zeros(W.shape)
    grad_b = np.zeros(b.shape)
    # print('grad X',grad_X.shape)
    # print('grad w',grad_W.shape)
    # print('grad b',grad_b.shape)
    # print('dela ',delta.shape)

    delta = delta * activation_deriv(post_act)

    grad_X = delta @ W.T
    grad_W = X.T @ delta
    for i in range(X.shape[0]):
        grad_b += delta[i,:]


    # store the gradients
    params['grad_W' + name] = grad_W
    params['grad_b' + name] = grad_b
    return grad_X

############################## Q 2.4 ##############################
# split x and y into random batches
# return a list of [(batch1_x,batch1_y)...]
def get_random_batches(x,y,batch_size):
    batches = []
    aval = range(x.shape[0])
    while len(aval) > 0:
        ind = np.random.randint(0, len(aval), batch_size)
        selected = [aval[i] for i in ind]
        batch_x = [x[i] for i in selected]
        batch_y = [y[i] for i in selected]
        batches.append((np.array(batch_x), np.array(batch_y)))
        aval = list(set(aval) - set(selected))



    ##########################
    ##### your code here #####
    ##########################



    return batches
