...
# Q 2.2.1
# implement sigmoid
test = sigmoid(np.array([-1000,1000],  dtype = np.float128))
    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                par1 = copy.deepcopy(params)
                par1[k][i,j] +=  eps
                h1 = forward(xb, par1, 'layer1')
                probs = forward(h1, par1, 'output', softmax)

                loss1, acc1 = compute_loss_and_acc(yb, probs)

                par2 = copy.deepcopy(params)
                par2[k][i,j] -=  eps
                h1 = forward(xb, par2, 'layer1')
                probs = forward(h1, par2, 'output', softmax)
