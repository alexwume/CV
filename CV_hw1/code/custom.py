...
def distance_to_set(word_hist, train_histograms):
    '''
    ...
    * sim: numpy.ndarray of shape (N)
    ...
    # ----- TODO -----
    # train_labels = train_system['labels']
    # train_histograms = train_system ['features']


    word_hist = np.tile(word_hist,(train_histograms.shape[0],1))
    diff = abs(word_hist - train_histograms)
    intersection = ma.masked_less_equal(diff,5)
    print(intersection)
    # intersection = np.minimum (word_hist,histograms)
    sim = np.sum(1*intersection, axis = 1)
    return sim
