...
def main():
    ...
    # img_path = join(opts.data_dir, 'highway/sun_apccrwjexoyshfqe.jpg')
    # img = Image.open(img_path)
    # img = np.array(img).astype(np.float32)/255
    ...
    print(conf)
    print(accuracy)
    np.savetxt(join(opts.out_dir, 'confmat_default_setting.csv'), conf, fmt='%d', delimiter=',')
    np.savetxt(join(opts.out_dir, 'accuracy_default_setting.txt'), [accuracy], fmt='%g')
